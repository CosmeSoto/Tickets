...
export default function ReportsPage() {
  ...
  const [activeTab, setActiveTab] = useState<
    'executive' | 'technicians' | 'trends' | 'sla' | 'satisfaction'
  >('executive')
      ...
      <div className='grid grid-cols-2 sm:grid-cols-4 gap-3'>
        ...
        <Card>
          <CardContent className='pt-4 pb-3'>
            ...
            <p
              className={`text-2xl font-bold mt-1 ${resolutionRate >= 80 ? 'text-green-600' : resolutionRate >= 50 ? 'text-yellow-600' : 'text-red-600'}`}
            >
              {resolutionRate}%
            </p>
          </CardContent>
              ...
              {data.length > 1 && (
                <tfoot>
                  <tr className='border-t-2 bg-muted/50 font-semibold'>
                    ...
                    <td className='p-3 text-right'>
                      <span
                        className={`font-semibold ${slaColor(
                          data.length > 0
                            ? Math.round(
                                (data.reduce((s, r) => s + r.slaComplianceRate, 0) / data.length) *
                                  10
                              ) / 10
                            : 0
                        )}`}
                      >
                        {data.length > 0
                          ? Math.round(
                              (data.reduce((s, r) => s + r.slaComplianceRate, 0) / data.length) * 10
                            ) / 10
                          : 0}
                        %
                      </span>
...
function TechniciansTab({ data, loading }: { data: TechnicianPerformance[]; loading: boolean }) {
      ...
      <Card>
        <CardHeader>
          <div className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3'>
            ...
            <Select
              value={granularity}
              onValueChange={v => onGranularityChange(v as typeof granularity)}
            >
...
function SLATab({ data, loading }: { data: SLAComplianceRow[]; loading: boolean }) {
  if (loading) return <TabLoadingState />
  if (data.length === 0)
    return (
                    ...
                    {priorities.map(priority => {
                      ...
                      return (
                        <tr key={priority} className='border-b hover:bg-muted/30 transition-colors'>
                          <td className='p-3'>
                            <span
                              className={`px-2 py-0.5 rounded text-xs font-medium ${priorityColor(priority)}`}
                            >
                ...
                <tbody>
                  {data.byFamily.map(row => (
                    <tr key={row.familyId} className='border-b hover:bg-muted/30 transition-colors'>
                      ...
                      <td className='p-3 text-right'>
                        <span
                          className={`font-semibold ${row.satisfactionRate >= 80 ? 'text-green-600' : row.satisfactionRate >= 60 ? 'text-yellow-600' : 'text-red-600'}`}
                        >
