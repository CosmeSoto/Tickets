...
export default function ContactSupportPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!config?.supportEmail) {
      toast({
        title: 'Error',
        description: 'No se ha configurado el email de soporte.',
        variant: 'destructive',
      })
      return
    }

    const subject = encodeURIComponent(`[Soporte] ${formData.subject}`)
    const body = encodeURIComponent(
      `Categoría: ${formData.category}\nPrioridad: ${formData.priority}\n\n${formData.message}`
    )
    const mailtoUrl = `mailto:${config.supportEmail}?subject=${subject}&body=${body}`
  ...
  const getCurrentStatus = () => {
    ...
    const dayName = ['sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday', 'saturday'][
      now.getDay()
    ] as keyof typeof config.supportHours
              ...
              <CardContent className='space-y-3'>
                {Object.entries(config.supportHours).map(([day, hours]) => (
                  <div key={day} className='flex justify-between'>
                    ...
                    <span
                      className={`text-sm font-medium ${hours === 'closed' ? 'text-red-600' : ''}`}
                    >
