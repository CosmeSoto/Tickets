...
function CatalogsContent() {
  ...
  useEffect(() => {
    ...
    if (!isAdmin && session.user.role !== 'TECHNICIAN') {
  ...
  const {
    ...
    loading,
    reload: fetchItems,
  } = useFetch<CatalogItem>(catalog.apiGet, {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast({
          title: catalog.deleteVerb === 'desactivar' ? 'Desactivado' : 'Eliminado',
        ...
        toast({
          title: 'Error',
          description: err.error || 'No se pudo completar la acción',
          variant: 'destructive',
        })
      }
    } catch {
  ...
  const { exportCSV, exportExcel, exportPDF, exporting } = useExport({
    ...
    columns: [
      ...
      ...(catalog.hasSymbol
        ? [{ key: 'symbol', label: 'Símbolo', format: (v: any) => v ?? '' }]
        : []),
      ...(catalog.hasFamily
        ? [{ key: 'family', label: 'Área', format: (v: any) => v?.name ?? 'Global' }]
        : []),
      ...(catalog.hasLocation
        ? [{ key: 'location', label: 'Ubicación', format: (v: any) => v ?? '' }]
        : []),
  ...
  // Sort de la tabla
  const { sorted: sortedItems, sortKey, sortDir, toggleSort } = useTableSort(items, 'name')
    ...
    <ModuleLayout title='Catálogos' subtitle='Gestiona los tipos y configuraciones del inventario'>
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={7} className='text-center py-8 text-muted-foreground'>
                    No hay elementos. Crea el primero.
                  </TableCell>
                </TableRow>
              ) : (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent aria-describedby={undefined}>
          <DialogHeader>
            <DialogTitle>
              {editingItem ? `Editar ${catalog.label.slice(0, -1)}` : `Nuevo en ${catalog.label}`}
            </DialogTitle>
          </DialogHeader>
          <form onSubmit={handleSubmit} className='space-y-4'>
            ...
            {catalog.hasFamily && (
              <div className='space-y-1'>
                ...
                />
                <p className='text-xs text-muted-foreground'>
                  Sin familia: visible en todas las familias. Con familia: aparece solo en esa
                  familia.
                </p>
              </div>
            )}
