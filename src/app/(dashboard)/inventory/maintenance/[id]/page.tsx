...
export default function MaintenanceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchMaintenance = async () => {
    try {
      ...
      toast({
        title: 'Error',
        description: 'No se pudo cargar el mantenimiento',
        variant: 'destructive',
      })
    } finally {
      setLoading(false)
  ...
  const handleAccept = async () => {
    try {
      ...
      toast({
        ...
        description: 'Has confirmado el mantenimiento programado.',
      })
      fetchMaintenance()
    } catch (e) {
      toast({
    ...
    <div className='space-y-6'>
        ...
        <div className='flex items-center gap-4'>
          ...
          <div>
            ...
            <p className='text-muted-foreground'>
              {maintenance.equipment.code} — {maintenance.equipment.brand}{' '}
                ...
                return (
                  <div key={step} className='flex items-center flex-1'>
                    <div className='flex flex-col items-center gap-1'>
                      ...
                      <span
                        className={`text-xs text-center ${current ? 'font-semibold text-primary' : done ? 'text-green-600' : 'text-muted-foreground'}`}
                      >
                        {cfg.label}
                      </span>
                    </div>
      ...
      {status === 'CANCELLED' && (
        <Alert className='border-border bg-muted'>
          ...
          <AlertDescription className='text-muted-foreground'>
            Este mantenimiento fue cancelado.
          </AlertDescription>
        </Alert>
      )}
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
        ...
        <Card>
          <CardHeader>
            <CardTitle className='flex items-center gap-2'>
              <Wrench className='h-5 w-5' /> Detalle
            </CardTitle>
          </CardHeader>
            ...
            {maintenance.acceptedAt && (
              <div className='flex justify-between items-center'>
                ...
                <span className='text-sm'>
                  {new Date(maintenance.acceptedAt).toLocaleDateString('es-ES')}
                </span>
              </div>
            )}
            {maintenance.completedAt && (
            ...
            {maintenance.partsReplaced?.length > 0 && (
              <div>
                ...
                <div className='flex flex-wrap gap-1 mt-1'>
                  {maintenance.partsReplaced.map((p, i) => (
        ...
        <Card>
          <CardHeader>
            <CardTitle className='flex items-center gap-2'>
              <Package className='h-5 w-5' /> Equipo y Personas
            </CardTitle>
          </CardHeader>
          <CardContent className='space-y-3'>
            <div className='flex justify-between items-center'>
              ...
              <Button
                ...
                onClick={() => router.push(`/inventory/equipment/${maintenance.equipment.id}`)}
              >
            ...
            <div className='flex justify-between items-center'>
              ...
              <span className='text-xs'>
                {new Date(maintenance.createdAt).toLocaleString('es-ES')}
              </span>
            </div>
          </CardContent>
        </Card>
          ...
          <div className='space-y-4'>
            ...
            <div>
              ...
              <Textarea
                ...
                rows={2}
                placeholder='Ej: El técnico se presentará a las 9am...'
              />
            </div>
          </div>
          <DialogFooter>
          ...
          <div className='space-y-4'>
            <div className='space-y-2'>
              ...
              <div className='grid gap-2'>
                ...
                >
                  <Warehouse
                    className={`h-5 w-5 flex-shrink-0 ${returnTo === 'available' ? 'text-primary' : 'text-muted-foreground'}`}
                  />
                  <div>
                    <p className='font-medium text-sm'>Enviar a bodega</p>
                    <p className='text-xs text-muted-foreground'>
                      El equipo queda disponible para asignación
                    </p>
                  </div>
                </button>
                {assignedUser && (
                  ...
                  >
                    <UserCheck
                      className={`h-5 w-5 flex-shrink-0 ${returnTo === 'previous_user' ? 'text-primary' : 'text-muted-foreground'}`}
                    />
                    <div>
                      <p className='font-medium text-sm'>Reasignar a {assignedUser.name}</p>
                      <p className='text-xs text-muted-foreground'>
                        El equipo vuelve al usuario que lo tenía
                      </p>
                    </div>
                  </button>
                )}
            ...
            {isAdminOrTech && (
              <>
                ...
                <div>
                  <Label>Partes reemplazadas (separadas por coma)</Label>
                  <Input
                    placeholder='Ej: Disco SSD, Batería'
                    value={completeParts}
