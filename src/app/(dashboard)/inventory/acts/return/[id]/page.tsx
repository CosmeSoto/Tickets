    ...
    <ModuleLayout title={`Acta ${folio}`} subtitle={`Acta de devolución · ${statusCfg.label}`}>
      <div className='max-w-4xl mx-auto space-y-6'>
        ...
        <div className='flex items-center justify-between'>
          <button
            onClick={() => router.push('/inventory/acts?tab=return')}
        ...
        <Card>
          ...
          <CardContent>
            <p className='text-sm text-muted-foreground flex items-center gap-2'>
              ...
              {isSuperAdmin && !isReturner && !isReceiver && (
                <span>
                  Viendo como <strong>Super Administrador</strong>
                </span>
              )}
              {isAdmin && !isSuperAdmin && !isReturner && !isReceiver && (
                <span>
                  Viendo como <strong>Administrador</strong>
                </span>
              )}
              {accessLevel === 'manager' && !isReturner && !isReceiver && (
                <span>
                  Viendo como <strong>Gestor de Inventario</strong>
                </span>
              )}
              {isReturner && isReceiver && (
                <span>
                  Eres el <strong>Entregador y Receptor</strong>
                </span>
              )}
              {isReturner && !isReceiver && (
                <span>
                  Eres quien <strong>devuelve</strong> el equipo
                </span>
              )}
              {isReceiver && !isReturner && (
                <span>
                  Eres quien <strong>recibe</strong> la devolución
                </span>
              )}
            </p>
          </CardContent>
          ...
          <CardContent>
            <div className='grid gap-4 sm:grid-cols-2 md:grid-cols-3'>
              {[
                ...
                [
                  'N° de Serie',
                  act.equipmentSnapshot?.serialNumber ??
                    act.assignment?.equipment?.serialNumber ??
                    '—',
                ],
                ...
                [
                  ...
                  CONDITION_LABELS[act.returnCondition] ?? act.returnCondition ?? '—',
                ],
              ].map(([label, value]) => (
        ...
        {canDownload && (
          <Card className='border-green-200 dark:border-green-500/40 bg-green-50/30 dark:bg-green-500/10'>
            <CardHeader className='pb-3'>
              ...
              <CardDescription>
                Acta firmada digitalmente. Puedes previsualizar o descargar el PDF oficial.
              </CardDescription>
            </CardHeader>
            <CardContent>
        ...
        {act.status === 'PENDING' && canSign && (
          <Card className='border-yellow-300 dark:border-yellow-500/40 bg-yellow-50/30 dark:bg-yellow-500/10'>
            <CardHeader className='pb-3'>
              ...
              <CardDescription>
                Confirma que recibiste el equipo devuelto en las condiciones indicadas
              </CardDescription>
            </CardHeader>
            <CardContent>
      ...
      <AlertDialog open={showAcceptDialog} onOpenChange={setShowAcceptDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Al confirmar, certificas que recibiste el equipo devuelto. Esta acción genera una
          ...
          <div className='flex items-start gap-3 px-1 py-2'>
            ...
            <Label htmlFor='terms' className='text-sm leading-relaxed cursor-pointer'>
              Confirmo que he inspeccionado el equipo devuelto y que la información del acta es
              correcta.
            </Label>
          </div>
          <AlertDialogFooter>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Estás a punto de eliminar el acta <strong>{folio}</strong>. Esta acción es
              irreversible. La auditoría quedará registrada.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
