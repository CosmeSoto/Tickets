  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast({
          title: 'Error',
          description: data.message || 'No se pudo enviar el email',
          variant: 'destructive',
        })
      }
    } catch {
  ...
  return (
    <AuthLayout>
      <AuthCard>
        ...
        <AuthStep stepKey={step} className='space-y-4'>
          {step === 'email' && (
            <form onSubmit={handleSubmit} className='space-y-4'>
              ...
              <AuthPressable disabled={isLoading || !email.trim()}>
                <Button
                  type='submit'
            ...
            <div className='space-y-4'>
              <Alert>
                ...
                <AlertDescription>
                  ...
                  <strong>{oauthProvider === 'google' ? 'Google' : 'Microsoft'}</strong>. Inicia
        ...
        <motion.div
          ...
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          transition={{ duration: reduceMotion ? 0 : 0.35, ease: AUTH_EASE, delay: 0.15 }}
        >
          <Link
            href='/login'
