...
const inventorySettingsSchema = z.object({
  technician_can_manage_equipment: z.boolean(),
  inventory_technician_ids: z.array(z.string()).optional().default([]),
...
// Campos que son JSON (arrays/objects)
const JSON_FIELDS = ['inventory_technician_ids']
// Campos booleanos
const BOOLEAN_FIELDS = [
  'technician_can_manage_equipment',
...
export async function GET(_request: NextRequest) {
  try {
    ...
    const dbSettings = await prisma.system_settings.findMany({
      where: {
        key: { in: settingsKeys.map(k => `inventory.${k}`) },
      },
    })

    // Construir objeto con defaults
...
function getSettingDescription(key: string): string {
  const descriptions: Record<string, string> = {
    technician_can_manage_equipment: 'Permite a los técnicos gestionar equipos',
    inventory_technician_ids: 'IDs de técnicos con acceso al inventario',
