...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    // Calcular promedios
    const sumResponseTime = (ratings as any[]).reduce(
      (sum: number, r: any) => sum + r.responseTime,
      0
    )
    const sumTechnicalSkill = (ratings as any[]).reduce(
      (sum: number, r: any) => sum + r.technicalSkill,
      0
    )
    const sumCommunication = (ratings as any[]).reduce(
      (sum: number, r: any) => sum + r.communication,
      0
    )
    const sumProblemResolution = (ratings as any[]).reduce(
      (sum: number, r: any) => sum + r.problemResolution,
      0
    )
