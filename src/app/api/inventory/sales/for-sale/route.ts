...
/**
 * GET /api/inventory/sales/for-sale
 * Obtiene equipos actualmente en venta
 */
  ...
  try {
    ...
    if (!session.user.canManageInventory && session.user.role !== 'ADMIN') {
      return NextResponse.json(
        { error: 'No tiene permisos para ver equipos en venta' },
        { status: 403 }
      )
    }
    ...
    const result = await SalesManagerService.getForSaleEquipment({
      familyId,
      modelId,
      search,
      page,
