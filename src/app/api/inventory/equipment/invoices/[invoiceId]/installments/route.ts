/**
 * GET  /api/inventory/equipment/invoices/[invoiceId]/installments — lista abonos
 * POST /api/inventory/equipment/invoices/[invoiceId]/installments — registra un
 *      pago (completo o parcial) — ver EquipmentInvoiceService.registerPayment.
 */
...
async function resolveAccess(
  ...
  await assert(toInventoryAccessUser(session.user), 'EQUIPMENT', invoice.equipment.id)
  return invoice
}
...
export async function GET(
  ...
  try {
    ...
    const installments = await EquipmentInvoiceService.listInstallments(invoiceId)
    return NextResponse.json({ installments })
  } catch (error) {
    console.error('[GET /equipment/invoices/[id]/installments]', error)
...
export async function POST(
  ...
  try {
    ...
    const invoice = await EquipmentInvoiceService.registerPayment(
      invoiceId,
      {
    ...
    console.error('[POST /equipment/invoices/[id]/installments]', error)
