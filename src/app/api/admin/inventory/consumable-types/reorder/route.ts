/**
 * PATCH /api/admin/inventory/consumable-types/reorder
 * Body: { ids: string[] }
 */
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    return persistCatalogOrder('consumable_types', parsed.data.ids)
  } catch (error) {
    console.error('[PATCH consumable-types/reorder]', error)
    return NextResponse.json({ error: 'Error al reordenar tipos' }, { status: 500 })
  }
}
