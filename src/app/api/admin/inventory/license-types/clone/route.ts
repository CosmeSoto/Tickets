/**
 * POST /api/admin/inventory/license-types/clone
 * Copia un tipo de licencia (con todos sus atributos) a otra familia.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    const newType = await prisma.$transaction(async tx => {
      const maxOrder = await tx.license_types.aggregate({
