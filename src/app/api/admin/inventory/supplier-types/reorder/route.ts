/**
 * PATCH /api/admin/inventory/supplier-types/reorder
 * Body: { ids: string[] }
 */
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    return persistCatalogOrder('supplier_types', parsed.data.ids)
  } catch (error) {
    console.error('[PATCH supplier-types/reorder]', error)
    return NextResponse.json(
      { error: 'Error al reordenar tipos de proveedor' },
      { status: 500 }
    )
  }
}
