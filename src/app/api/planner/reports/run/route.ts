...
export async function POST(request: NextRequest) {
  try {
    ...
    // combineDateAndTime no valida el formato — un string cualquiera produce
    // un Invalid Date que Prisma rechaza con un 500 sin mensaje útil. Se
    // valida acá, antes de llegar a la consulta.
    if (body.from !== undefined && body.from !== '' && !DATE_ONLY.test(body.from)) {
      return NextResponse.json(
        { error: 'Fecha "desde" inválida — usa el formato AAAA-MM-DD' },
        { status: 400 }
      )
    }
    if (body.to !== undefined && body.to !== '' && !DATE_ONLY.test(body.to)) {
      return NextResponse.json(
        { error: 'Fecha "hasta" inválida — usa el formato AAAA-MM-DD' },
        { status: 400 }
      )
    }
    ...
    console.error('[API] Error generando reporte de tareas:', error)
    return NextResponse.json(
      {
