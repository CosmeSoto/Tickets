...
function PasswordStrength({ password }: { password: string }) {
  ...
  const textColor =
    pct === 33 ? 'text-destructive' : pct === 66 ? 'text-yellow-600' : 'text-green-600'
  ...
  return (
    <motion.div
      ...
      transition={{ duration: reduceMotion ? 0 : 0.2, ease: AUTH_EASE }}
    >
      ...
      <div className='w-full bg-muted rounded-full h-1.5 overflow-hidden'>
        <motion.div
          ...
          initial={false}
          animate={{ width: `${pct}%` }}
          transition={{ duration: reduceMotion ? 0 : 0.35, ease: AUTH_EASE }}
        />
      </div>
    </motion.div>
  )
...
function ResetPasswordForm() {
  ...
  useEffect(() => {
    ...
    fetch('/api/auth/validate-reset-token', {
      ...
      .then(r => r.json())
      .then(d => {
        if (d.valid) setIsValidToken(true)
        else setError(d.message || 'Token inválido o expirado')
      })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!/[A-Z]/.test(password) || !/[a-z]/.test(password) || !/[0-9]/.test(password)) {
      setError('Debe contener mayúsculas, minúsculas y números')
      return
    }
          ...
          {!isValidating && !success && isValidToken && (
            <div className='space-y-6'>
              ...
              <form onSubmit={handleSubmit} className='space-y-4'>
                ...
                <AuthPressable disabled={isLoading || !password || !confirmPassword}>
                  <Button
                    type='submit'
