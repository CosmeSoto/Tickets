...
export class ConfigService {
  ...
  static async getHelpSystemConfig(): Promise<HelpSystemConfig> {
    // Obtener todas las configuraciones del sistema (help. y las generales)
    const settings = await prisma.system_settings.findMany()
    // Obtener también la configuración de la landing page
    const landingPage = await prisma.landing_page_content.findFirst({
      where: { id: 'default' },
    })
    ...
    for (const setting of settings) {
      if (setting.key.startsWith('help.')) {
    ...
    // Buscar también la configuración general (supportEmail y systemName)
  ...
  static async updateHelpSystemConfig(config: Partial<HelpSystemConfig>): Promise<void> {
    ...
    for (const [key, value] of Object.entries(config)) {
      ...
      // Si es supportEmail o companyName, también actualizar la configuración general
      if (key === 'supportEmail') {
        updates.push(
          prisma.system_settings.upsert({
