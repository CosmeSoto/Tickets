...
export async function restoreFromJSON(
  ...
  const restoreOrder = [
    ...
    'pages',
    'oauth_configs',
    // Bloque de inventario completo, en el mismo orden validado que usa la
    // restauración selectiva por módulo (ver INVENTORY_MODULE_RESTORE_ORDER) —
    // antes este camino "legacy" tenía su propia lista, incompleta (le
    // faltaban ~25 tablas agregadas al módulo con el tiempo, como
    // equipment_batches, license_batches, equipment_models, equipment_sales,
    // facturas y sus cuotas) y con inconsistencias de orden FK.
    ...INVENTORY_MODULE_RESTORE_ORDER,
    'user_family_access',
    'user_settings',
