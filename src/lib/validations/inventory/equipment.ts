...
const baseEquipmentSchema = z.object({
  ...
  saleListingPrice: z.preprocess(
    val => (val === '' || val === undefined || val === null || Number.isNaN(val) ? undefined : val),
    z.number().positive('El precio de venta debe ser positivo').optional()
  ),
  ...
  rentalContactEmail: z.string().email('Email de contacto inválido').optional(),

  rentalContactPhone: z
    .string()
