...
export async function generateDecommissionActPDF(data: DecommissionActPDFData): Promise<any> {
  ...
  let cy = card(
    ...
    colW,
    assetH,
    data.request.assetType === 'EQUIPMENT' ? 'EQUIPO DADO DE BAJA' : 'LICENCIA DADA DE BAJA'
  )
  if (data.equipment) {
    ...
    if (data.request.condition) {
      cy = row(
        ...
        colW,
        'Condición',
        CONDITION_LABELS[data.request.condition] || data.request.condition
      )
    }
  ...
  const motivoH = 22 + 36
  const my = card(col1X, motivoY, colW, motivoH, 'MOTIVO DE BAJA')
  doc
    .fontSize(8)
    .font('Helvetica')
    .fillColor(C.text)
  ...
  const fechasH = 42
  const fy = card(col2X, fechasY, colW, fechasH, 'FECHA DE BAJA')
