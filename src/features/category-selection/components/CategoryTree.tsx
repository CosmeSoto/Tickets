  ...
  const renderNode = (node: CategoryTreeNodeData, depth: number = 0): React.ReactNode => {
    ...
    return (
      <div key={node.category.id} className='select-none'>
        <div
          className={cn(
            'flex items-start gap-1 p-2 rounded-md transition-colors group',
        ...
        >
          ...
          <button
            ...
            className='flex-1 flex items-start gap-2 text-left min-w-0 py-0.5'
            ...
            role='treeitem'
          >
            {/* Color indicator — aligned to first line */}
            <span
            ...
            {/* Name + description inline — allows wrapping on mobile */}
            <span className='flex-1 min-w-0'>
              ...
              {/* Description shown inline on mobile (no hover required) */}
