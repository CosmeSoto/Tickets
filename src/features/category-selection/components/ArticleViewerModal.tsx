      ...
      if (response.ok) {
        ...
        if (article) {
          setArticle({
            ...
            notHelpfulVotes: !isHelpful ? article.notHelpfulVotes + 1 : article.notHelpfulVotes,
          })
        }
      }
    } catch (err) {
          ...
          {article && !isLoading && !error && (
            <>
              <DialogHeader>
                ...
                {article.summary && (
                  <DialogDescription className='text-base'>{article.summary}</DialogDescription>
                )}
              ...
              <DialogFooter className='flex-col sm:flex-row gap-2 pt-4'>
                <Button variant='default' onClick={handleMarkResolved} className='w-full sm:w-auto'>
