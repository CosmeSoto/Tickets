...
import { extractKeywords } from '../config/fuse.config'
...
export function useCategorySuggestions({
  ...
  const categoriesMap = useMemo(() => {
    return new Map<string, Category>(categories.map(cat => [cat.id, cat]))
  }, [categories])
  ...
  const generateReason = useCallback((matchedKeywords: string[], _score: number): string => {
