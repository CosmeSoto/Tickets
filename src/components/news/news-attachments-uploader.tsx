 ...
 *
 * Componente para subir y gestionar archivos adjuntos de una noticia.
 * Soporta:
 *  - Drag & drop en desktop
 *  - Botón de cámara/galería en móvil (via FileInputWithCamera)
 *  - Múltiples archivos simultáneos
 *  - Preview de imágenes antes de subir
 *  - Lista de adjuntos ya subidos con opción de eliminar
 *
 * Uso:
 *   <NewsAttachmentsUploader
 *     newsId="uuid"                    // requerido solo en modo edición
 *     pendingFiles={files}             // archivos pendientes (antes de guardar)
 *     onPendingFilesChange={setFiles}  // actualiza lista de pendientes
 *     uploadedAttachments={[...]}      // adjuntos ya en BD (modo edición)
 *     onDeleteUploaded={fn}            // elimina adjunto de BD
 *   />
 */

import { useCallback, useState, useRef } from 'react'
import { Upload, X, FileText, Image, Film, File, Camera, Trash2, Paperclip } from 'lucide-react'
...
interface PendingFile {
  file: File
  preview: string | null // URL de objeto para imágenes
  id: string // id temporal para key
}

interface NewsAttachmentsUploaderProps {
  /** Archivos pendientes de subir (controlado por el padre) */
  pendingFiles: PendingFile[]
  onPendingFilesChange: (files: PendingFile[]) => void
  /** Adjuntos ya guardados en BD (solo en modo edición) */
...
export function NewsAttachmentsUploader({
  pendingFiles,
  ...
  const processFiles = useCallback(
    ...
    [pendingFiles, onPendingFilesChange, totalFiles, maxFiles, maxSizeMB]
  )
            ...
            <div className='flex flex-col items-center justify-center gap-2 py-6 px-4 text-center'>
              ...
              <div>
                ...
                <p className='text-xs text-muted-foreground mt-0.5'>
                  Imágenes, PDF, Word, Excel · Máx {maxSizeMB}MB por archivo
                </p>
              </div>
            ...
            >
              ...
              <div className='w-10 h-10 rounded-md overflow-hidden flex-shrink-0 bg-muted flex items-center justify-center'>
                {att.mimeType.startsWith('image/') ? (
                  ...
                  <img
                    src={`/api/admin/news/attachment-preview/${att.id}`}
