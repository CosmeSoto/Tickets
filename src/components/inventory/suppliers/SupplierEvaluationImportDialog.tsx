...
export function SupplierEvaluationImportDialog({
  ...
  const handleImport = async () => {
    ...
    if (!defaultFamilyId && !isSuperAdmin) {
      toast({
        title: 'Selecciona un área por defecto',
        description: 'Se usa solo para proveedores nuevos detectados en el archivo',
        variant: 'destructive',
      })
      return
    }
    ...
    <div className='space-y-4'>
      <div className='rounded-md border border-dashed p-4 text-center space-y-2'>
        ...
        <p className='text-xs text-muted-foreground'>
          ...
          Tiempo de entrega, Precio, Referencias, Equipo (0-5).
        </p>
      </div>
      ...
      {rows.length > 0 && (
        <>
          <div className='space-y-1'>
            ...
            />
            <p className='text-xs text-muted-foreground'>
              Se aplica solo a proveedores que no existan aún en el sistema (se buscan por nombre
              exacto). Los ya existentes conservan su área actual.
            </p>
          </div>
