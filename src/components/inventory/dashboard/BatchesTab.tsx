...
interface BatchItem {
  ...
  model: { brand: string; model: string; type?: { name: string } | null }
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // El API devuelve { batches, pagination } o array directo
        ...
        <div className='grid grid-cols-2 sm:grid-cols-4 gap-3'>
          ...
          <div className='rounded-lg border bg-card p-3 text-center'>
            <p className='text-2xl font-bold text-green-600'>
              {batches.reduce((s, b) => s + b.metrics.available, 0)}
            </p>
            <p className='text-xs text-muted-foreground'>Disponibles</p>
          </div>
          <div className='rounded-lg border bg-card p-3 text-center'>
            <p className='text-2xl font-bold text-blue-600'>
              {batches.reduce((s, b) => s + b.metrics.assigned, 0)}
            </p>
                ...
                <div className='flex items-start justify-between gap-4'>
                  ...
                  <div className='shrink-0 flex items-center gap-3 text-xs'>
                    <div className='flex items-center gap-1 text-green-600' title='Disponibles'>
                      ...
                      <span className='font-semibold'>{batch.metrics.available}</span>
                    </div>
                    <div className='flex items-center gap-1 text-blue-600' title='Asignados'>
                      <UserCheck className='h-3.5 w-3.5' />
                      <span className='font-semibold'>{batch.metrics.assigned}</span>
                    </div>
                    {batch.metrics.maintenance > 0 && (
                      <div
