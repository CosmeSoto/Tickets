...
import {
  Upload, Download, CheckCircle, AlertTriangle, FileText,
...
interface PreviewRow {
  ...
  typeName: string; acquisitionMode: string; status: string
  warehouseName?: string; supplierName?: string
}
interface ImportResult {
  total: number; created: number; updated: number; skipped: number
  errors: ImportError[]; preview?: PreviewRow[]
}
...
export function EquipmentImportModal({
  open, onOpenChange, onSuccess, familyId, familyName,
  ...
  const reset = () => {
    ...
    setResult(null); setLoading(false); setMode('add')
  }
  ...
  const handleFile = (f: File) => {
    ...
    if (!valid) {
      toast({ title: 'Formato inválido', description: 'Se aceptan CSV o Excel (.xlsx)', variant: 'destructive' })
      return
    }
    if (f.size > maxFileSizeMB * 1024 * 1024) {
      toast({ title: 'Archivo muy grande', description: `El archivo supera el límite de ${maxFileSizeMB} MB configurado en el sistema`, variant: 'destructive' })
      return
    }
    setFile(f)
  }
  ...
  const handlePreview = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/inventory/equipment/import', { method: 'POST', body: buildFormData(true) })
  ...
  const handleImport = async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/inventory/equipment/import', { method: 'POST', body: buildFormData(false) })
      const data = await res.json()
      if (!res.ok) { toast({ title: 'Error al importar', description: data.error, variant: 'destructive' }); return }
      setResult(data)
      setStep('done')
    } catch {
      toast({ title: 'Error de conexión', variant: 'destructive' })
        ...
        {step === 'upload' && (
          <div className='space-y-4'>
            ...
            <div className='flex items-center justify-between p-3 bg-muted/50 rounded-lg border'>
              <div className='flex items-center gap-2'>
                ...
                <div>
                  ...
                  <p className='text-xs text-muted-foreground'>Descarga el formato con ejemplos e instrucciones</p>
                </div>
              </div>
              ...
              <div className='grid grid-cols-2 gap-2'>
                {MODE_OPTIONS.map(opt => (
                  <button key={opt.value} type='button' onClick={() => setMode(opt.value)}
                    ...
                    <div className={cn('flex items-center gap-1.5 font-medium text-sm', mode === opt.value && 'text-primary')}>
                      {opt.icon}{opt.label}
                    </div>
            ...
            >
              ...
              {file ? (
                ...
                <div className='flex flex-col items-center gap-1.5'>
                  ...
                  <p className='text-xs text-muted-foreground'>CSV o Excel (.xlsx) · Máx. 500 equipos · {maxFileSizeMB} MB</p>
                </div>
              )}
            </div>
            ...
            <p className='text-xs text-muted-foreground p-2 bg-muted/30 rounded'>
              ...
              <span className='font-mono'>Marca</span>* ·{' '}
        ...
        {step === 'preview' && preview && (
          <div className='space-y-4'>
            <div className='grid grid-cols-3 gap-3'>
              ...
              <div className={cn('text-center p-3 rounded-lg border', errorRows > 0 ? 'bg-destructive/5 border-destructive/20' : 'bg-muted/50')}>
                <p className={cn('text-2xl font-bold', errorRows > 0 && 'text-destructive')}>{errorRows}</p>
            ...
            {preview.errors.length > 0 && (
              <div className='space-y-1 max-h-28 overflow-y-auto'>
                ...
                {preview.errors.map((e, i) => (
                  <div key={i} className='flex items-start gap-2 text-xs p-2 bg-destructive/5 rounded border border-destructive/10'>
            ...
            {preview.preview && preview.preview.length > 0 && (
              <div className='space-y-1'>
                <p className='text-xs font-medium text-muted-foreground'>Vista previa (primeros 5):</p>
                <div className='border rounded-lg divide-y max-h-40 overflow-y-auto'>
                  {preview.preview.slice(0, 5).map((row, i) => (
                    <div key={i} className='flex items-center gap-3 px-3 py-2'>
                      <div className='flex-1 min-w-0'>
                        <p className='text-sm font-medium truncate'>{row.brand} {row.model}</p>
                        <p className='text-xs text-muted-foreground font-mono'>{row.serialNumber}</p>
                      </div>
                      <div className='flex items-center gap-1.5 flex-shrink-0'>
                        <Badge variant='outline' className='text-xs h-5 px-1.5'>{row.typeName}</Badge>
        ...
        {step === 'done' && result && (
          <div className='space-y-3 text-center py-4'>
            ...
            <div className='flex items-center justify-center gap-4 text-sm'>
              {result.created > 0 && (
                <span className='text-green-600 dark:text-green-400 font-medium'>+{result.created} creados</span>
              )}
              {result.updated > 0 && (
            ...
            {result.errors.length > 0 && (
              <p className='text-xs text-destructive'>{result.errors.length} filas con errores no se importaron</p>
            )}
          </div>
        )}
          ...
          {step === 'preview' && (
            <>
              ...
              <Button onClick={handleImport} disabled={loading || validRows === 0}>
                {loading && <Loader2 className='h-4 w-4 mr-2 animate-spin' />}
                {loading ? 'Importando...' : `Importar ${validRows} equipo${validRows !== 1 ? 's' : ''}`}
              </Button>
            </>
          )}
