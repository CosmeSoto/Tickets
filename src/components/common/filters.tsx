...
export function Filters({
  ...
  const visibleFilters = useMemo(() => {
    return filterConfigs.filter(config => !config.roles || config.roles.includes(userRole))
  ...
  const handleFilterChange = useCallback(
    (key: string, value: any, config: FilterConfig) => {
            ...
            <div key={config.key} className='space-y-2' style={{ width: config.width }}>
              ...
              >
                <SelectTrigger className='h-10'>
                  <SelectValue
                    placeholder={config.placeholder || `Seleccionar ${config.label.toLowerCase()}`}
                  />
                </SelectTrigger>
                <SelectContent>
              ...
              <div className='space-y-2'>
                ...
                >
                  <SelectTrigger className='h-10'>
                    <SelectValue
                      placeholder={
                        config.placeholder || `Seleccionar ${config.label.toLowerCase()}`
                      }
                    />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div className='grid grid-cols-2 gap-2'>
                <div className='space-y-1'>
                  ...
                  <DateInput
                    ...
                    onChange={e =>
                      handleFilterChange(
                        config.key,
                        { ...dateValue, start: e.target.value },
                        config
                      )
                    }
                    disabled={loading}
                ...
                <div className='space-y-1'>
                  ...
                  <DateInput
                    value={dateValue.end || ''}
                    onChange={e =>
                      handleFilterChange(config.key, { ...dateValue, end: e.target.value }, config)
                    }
                    disabled={loading}
                    className='h-10'
                    clearable
  ...
  if (variant === 'horizontal') {
    return (
      <div
        className={cn('flex items-center space-x-2 p-2 rounded-lg border', theme.border, className)}
      >
        ...
        {showActiveCount && activeFiltersCount > 0 && (
          <Badge className={theme.badge}>{activeFiltersCount}</Badge>
        )}
  ...
  return (
    <div
      className={cn('p-4 rounded-lg border space-y-4', theme.border, className)}
      style={{ height }}
    >
