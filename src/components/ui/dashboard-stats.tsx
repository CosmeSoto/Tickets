...
export function DashboardStats({ period = 'month', refreshInterval = 300 }: DashboardStatsProps) {
  ...
  }, [period, refreshInterval]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const loadStats = async () => {
    try {
      ...
      if (lastUpdated) {
        // Solo mostrar error si no es la primera carga
        toast({
            ...
            <div className='flex items-center justify-between'>
              <div>
                ...
                <div className='flex items-center mt-1'>
                  ...
                  <span
                    className={`text-xs ml-1 ${getTrendColor(stats.performance.resolutionTrend)}`}
                  >
                    {stats.performance.resolutionChange > 0 ? '+' : ''}
                    {stats.performance.resolutionChange}%
                  </span>
                </div>
              </div>
        ...
        <Card>
          ...
          <CardContent className='space-y-4'>
            <div className='space-y-3'>
              ...
              <Progress value={(stats.tickets.open / stats.tickets.total) * 100} className='h-2' />
            </div>
              ...
              <div className='space-y-2 max-h-40 overflow-y-auto'>
                {stats.technicians.workload.map(tech => (
                  ...
                  >
                    ...
                    <Badge variant='outline'>{tech.assignedTickets} tickets</Badge>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className='flex items-center justify-between p-3 bg-blue-50 rounded-lg'>
              <div>
                ...
                <div className='flex items-center space-x-2 mt-1'>
                  ...
                  <span
                    className={`text-sm font-bold ${getTrendColor(stats.performance.satisfactionTrend)}`}
                  >
                    {stats.performance.satisfactionChange > 0 ? '+' : ''}
                    {stats.performance.satisfactionChange}%
                  </span>
                </div>
              </div>
