          ...
          {assignments.length === 0 ? (
            ...
            <div className='space-y-3'>
              {assignments.map((assignment, index) => {
                const level =
                  PRIORITY_LEVELS[assignment.priority as keyof typeof PRIORITY_LEVELS] ||
                  PRIORITY_LEVELS[2]
                        ...
                        <div className={`p-3 rounded-lg ${level.bgColor}`}>
                          <p className={`text-xs ${level.textColor}`}>
                            {assignment.autoAssign ? (
                              <>
                                <strong>Nivel {level.label}:</strong> Recibirá tickets
                                automáticamente
                                {assignment.maxTickets && (
                                  <> (máximo {assignment.maxTickets} simultáneos)</>
                                )}
                              </>
                            ) : (
                              <>
                                <strong>Solo asignación manual:</strong> No recibirá tickets
                                automáticamente
                              </>
                            )}
                          </p>
            ...
            <div className='mt-4 p-4 bg-muted/50 rounded-lg space-y-2'>
              ...
              <div className='space-y-1'>
                <div className='flex items-start space-x-2 text-xs text-muted-foreground'>
                  ...
                  <span>
                    <strong>Principal:</strong> Recibe todos los tickets primero hasta llegar a su
                    límite
                  </span>
                </div>
                <div className='flex items-start space-x-2 text-xs text-muted-foreground'>
                  ...
                  <span>
                    <strong>Regular:</strong> Recibe tickets cuando Principal está ocupado
                  </span>
                </div>
                <div className='flex items-start space-x-2 text-xs text-muted-foreground'>
                  <Zap className='h-3 w-3 mt-0.5 text-orange-500' />
                  <span>
                    <strong>Respaldo:</strong> Solo recibe cuando Principal y Regular están ocupados
                  </span>
                </div>
              </div>
            </div>
      ...
      <Dialog open={editingIndex !== null} onOpenChange={open => !open && handleCancelSelection()}>
        ...
        >
          ...
          <div className='flex justify-end gap-2 pt-4 border-t'>
            ...
            <Button type='button' onClick={handleConfirmCategory} disabled={!tempCategoryId}>
              Confirmar Categoría
            </Button>
          </div>
        </DialogContent>
