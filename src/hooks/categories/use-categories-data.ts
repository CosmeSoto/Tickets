...
export function useCategoriesData(options: UseCategoriesDataOptions = {}) {
  ...
  const setToCache = useCallback(
    <T>(key: string, data: T, ttl = cacheTTL) => {
  ...
  const loadAvailableParents = useCallback(
    async (currentCategoryId?: string, familyId?: string) => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.success && Array.isArray(data.data)) {
            ...
            if (familyId) {
              availableAsParents = availableAsParents.filter(
                (cat: any) =>
                  cat.departments?.familyId === familyId || cat.departments?.family?.id === familyId
              )
            }

            const enrichedCategories = enrichCategoriesWithLevelName(availableAsParents).map(
              cat => ({
                ...cat,
                assignedTechnicians:
                  cat.technician_assignments?.map(ta => ({
                    id: ta.users.id,
                    name: ta.users.name,
                ...
                technicianStats: {
                  ...
                  highPriority:
                    cat.technician_assignments?.filter(ta => ta.priority <= 3).length || 0,
                },
              })
            )
