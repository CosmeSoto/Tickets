...
export function useCategoriesForm(options: UseCategoriesFormOptions = {}) {
  ...
  const handleEdit = useCallback((category: CategoryData) => {
    ...
    setFormData({
      ...
      priorityCeiling: category.priorityCeiling ?? 'MEDIUM',
  ...
  const calculateLevel = useCallback(
    (parentId: string | null, availableParents: CategoryData[]): number => {
    ...
    async (
      ...
      try {
        ...
        const apiData = {
          ...
          priorityCeiling: restFormData.priorityCeiling || null,
  ...
  const handleDelete = useCallback(
    async (invalidateCache: () => void, reload: () => Promise<void>) => {
