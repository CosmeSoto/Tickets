...
export function useMassActions<T>(options: MassActionOptions<T>): MassActionResult<T> {
  const {
    onBulkDelete,
    onBulkUpdate,
    onBulkActivate,
  ...
  const getDeletableItems = useCallback(
    (items: T[]): T[] => {
  ...
  const getUpdatableItems = useCallback(
    (items: T[]): T[] => {
  ...
  const bulkUpdate = useCallback(
    async (items: T[], updates: Partial<T>) => {
  ...
  const getSelectionSummary = useCallback(
    (items: T[]): string => {
      ...
      if (selectedItemsData.length === 1)
        return `1 elemento seleccionado: ${getItemName(selectedItemsData[0])}`
