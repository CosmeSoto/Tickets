/**
 * Dato semilla original de `help_faqs` — usado SOLO por `prisma/seed.ts` /
 * `prisma/ensure-help-faqs.ts` (ambos corren con `tsx` en el contenedor de
 * producción, donde solo se copia `prisma/`, no `src/`; importar desde
 * `src/features/help/data/faq-by-module.ts` rompía el seed en producción
 * con un "Cannot find module" silencioso). El tipo se duplica aquí a
 * propósito para que este archivo no dependa de nada fuera de `prisma/`.
 *
 * La fuente de verdad en tiempo de ejecución es la tabla `help_faqs`
 * (editable desde Configuración Sistema → Ayuda) — editar este array
 * después de la siembra inicial no tiene ningún efecto.
 */
