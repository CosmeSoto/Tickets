...
export async function seedCategories(prisma: PrismaClient, deptMap: Map<string, string>) {
  ...
  const suministros = await upsertCategory(
    prisma,
    {
      name: 'Suministros',
  ...
  console.log(
    `✅ Categorías Tecnología y Comunicaciones: ${counters.created} creadas, ${counters.updated} actualizadas (5 departamentos con jerarquía N1 → N2 → N3)`
  )
}
...
export async function seedCategoriesOtherFamilies(
  ...
  if (deptCivil) {
    const fallasCivil = await upsertCategory(
      prisma,
      {
        name: 'Falla o Daño',
    ...
    const solicCivil = await upsertCategory(
      prisma,
      {
  ...
  if (deptElec) {
    const fallasElec = await upsertCategory(
      prisma,
      {
    ...
    const solicElec = await upsertCategory(
      prisma,
      {
  ...
  if (deptSeg) {
    const incSeg = await upsertCategory(
      prisma,
      {
    ...
    const solicSeg = await upsertCategory(
      prisma,
      {
  ...
  if (deptLimp) {
    const solicLimp = await upsertCategory(
      prisma,
      {
  ...
  if (deptAdmin) {
    const solicAdmin = await upsertCategory(
      prisma,
      {
  ...
  console.log(
    `✅ Categorías otras familias: ${counters.created} creadas, ${counters.updated} actualizadas (Mantenimiento, Seguridad, Servicios, Administrativa)`
  )
}
